...
export function buildKolamSegments(pattern) {
  ...
  return [
    `M ${d1.x} ${d1.y} Q ${d1.x + 30} ${d1.y - 30} ${d2.x} ${d2.y}`,
    `M ${d2.x} ${d2.y} Q ${d2.x + 30} ${d2.y + 30} ${d3.x} ${d3.y}`,
    `M ${d3.x} ${d3.y} Q ${d3.x - 30} ${d3.y + 30} ${d1.x} ${d1.y}`,
  ];
}

export function buildKolamPath(pattern) {
  return buildKolamSegments(pattern).join(" ");
}
